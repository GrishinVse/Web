...
function clearArea() {
    var canvas = document.getElementById("genDots");
    var ctx = canvas.getContext("2d");
...
function genData() {
    ...
    for (let i = 0; i < dotsNum; i++) {
        let loc_data = []
        loc_data.push(Math.random() * (max - min) + min) // gen X
        loc_data.push(Math.random() * (max - min) + min) // gen Y
        data.push(loc_data)
    }
    ...
    with (ctx) {
        strokeStyle = "#ff0000";
        for (el in data) {
            //console.log(`${el} || X = ${data[el][0]} | Y = ${data[el][1]}`);
            let x = data[el][0];
            let y = data[el][1];
            beginPath()
    ...
    for (el in data) {
        ...
        console.log(`${el} || ${el % 2}`);
        if (el % 2 == 1) { // Нечетный
            let new_x = data[el][0] + dx;
            let new_y = data[el][1];
            console.log(`Нечетный || X = ${new_x} | Y = ${new_y}`);
            loc_data.push(new_x)
            loc_data.push(new_y)
        } else { // Четный
            let new_x = data[el][0];
            let new_y = data[el][1] - dy;
            console.log(`Четный || X = ${new_x} | Y = ${new_y}`);
            loc_data.push(new_x)
            loc_data.push(new_y)
        }
        new_data.push(loc_data)
    }
    ...
    with (ctx) {
        strokeStyle = "#6a329f";
        for (el in data) {
            let new_x = new_data[el][0];
            let new_y = new_data[el][1];
            beginPath()
                ctx.arc(new_x, new_y, 2, 0, Math.PI*2);
            stroke()
        }
    }
    ...
    with (ctx) {
        //strokeStyle = "#8d9c10";
        for (el in data) {
            strokeStyle = '#'+(0x1000000+(Math.random())*0xffffff).toString(16).substr(1,6);
            let x = data[el][0];
            let y = data[el][1];
            let new_x = new_data[el][0];
            let new_y = new_data[el][1];
